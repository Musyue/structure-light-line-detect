...
class StruLight:
    def __init__(self, u0, v0, pu, pv, f, alpha, x4_laser_ref, y4_laser_ref, z4_laser_ref, T1, urdfname, calibinfo,
                 lamda=0.9, ):
        self.u0 = u0
        self.v0 = v0
        self.f = f
    ...
    def Get_p_theta(self, n_uv, u1_desire, v1_desire, u2_desire, v2_desire, u1, v1, u2, v2):
        a_uv_desire = (n_uv * (u1_desire * v1_desire + u2_desire * v2_desire) - (u1_desire + u2_desire) * (
                    v1_desire + v2_desire)) / (n_uv * (u1_desire ** 2 + u2_desire ** 2) - (u1_desire + u2_desire) ** 2)
    ...
    def Get_p3_theta3(self, a_uv_desire, b_uv_desire, A11, A12, A14, A21, A22, A24, A31, A32, A34):
        ...
        s1_desire = (puv_desire - self.u0 * math.sin(theta_uv_desire) - self.v0 * math.cos(
            theta_uv_desire)) * self.pu * self.pv / (
                            self.f * ((self.pv * math.sin(theta_uv_desire)) ** 2 + (
                                self.pu * math.cos(theta_uv_desire)) ** 2) ** 0.5)
        s2_desire = math.sin(theta_uv_desire) / (
                    math.sin(theta_uv_desire) ** 2 + (self.pu / self.pv * math.cos(theta_uv_desire)) ** 2) ** 0.5
        s3_desire = math.cos(theta_uv_desire) / (
                    math.cos(theta_uv_desire) ** 2 + (self.pv / self.pu * math.sin(theta_uv_desire)) ** 2) ** 0.5
    ...
    def Get_vxvyvzwxwywz(self, theta_uv, theta_uv_desire, puv, puv_desire, a_uv_desire, b_uv_desire,
                         T_reflection_to_camera, T_camera_to_endeffector):
        T = T_reflection_to_camera
        A11, A12, A14, A21, A22, A24, A31, A32, A34 = self.GET_T_A(T)
        invA11, invA12, invA13, invA14, invA21, invA22, invA23, invA24, invA31, invA32, invA33, invA34 = self.GET_Inv_T(
            T)

        delta_theta, delta_puv = self.delta_puv(theta_uv, theta_uv_desire, puv, puv_desire)
        p3_desire, theta3_desire = self.Get_p3_theta3(a_uv_desire, b_uv_desire, A11, A12, A14, A21, A22, A24, A31, A32,
                                                      A34)
        ...
        s1 = (puv - self.u0 * math.sin(theta_uv) - self.v0 * math.cos(theta_uv)) * self.pu * self.pv / (
                    self.f * ((self.pv * math.sin(theta_uv)) ** 2 + (self.pu * math.cos(theta_uv)) ** 2) ** 0.5)
        # define line_jacob1
        ...
        t12 = 0
        t21 = self.pu * self.pv * (puv - self.u0 * math.cos(theta_uv) + self.v0 * math.sin(theta_uv)) / (
                    self.f * ((self.pv * math.sin(theta_uv)) ** 2 + (self.pu * math.cos(theta_uv)) ** 2) ** 0.5) \
              + self.pu * self.pv * (puv - self.u0 * math.sin(theta_uv) - self.v0 * math.cos(theta_uv)) * (
                          self.pu - self.pv) * math.sin(theta_uv) * math.cos(theta_uv) \
        ...
        # delta_theta = theta_uv - theta_uv_desire
        # delta_puv = puv - puv_desire
        delta_theta3 = (theta3 - theta3_desire) * 1
        delta_p3 = (p3 - p3_desire) * 1
        self.delta_theta_pub.publish(delta_theta3)
        self.delta_puv_pub.publish(delta_p3)
        ...
        vel_refelection = numpy.dot(-self.lamda * inv_line_jacob3, numpy.matrix([delta_theta3, delta_p3]).T)
        ...
        vel_refelection_new = [vx1.tolist()[0][0], vy1.tolist()[0][0], vz1.tolist()[0][0], wx1.tolist()[0][0],
                               wy1.tolist()[0][0], wz1.tolist()[0][0]]
        ...
        T_reflection_to_endeffector = numpy.dot(numpy.matrix(T_camera_to_endeffector).reshape(4, 4),
                                                numpy.matrix(T_reflection_to_camera).reshape(4, 4))
...
def main():
    ...
    T1 = [-0.7952719448436829, 0.02682308183882049, 0.6056591913174864, 0.134482874649, 0.05504174517916128,
          0.9980893937608886, 0.028070773941172033, 0.00201681390159, -0.6037490704210781, 0.05566043785932738,
          -0.7952288825395675, 0.571575563438, 0.0, 0.0, 0.0, 1.0]
    ...
    # print "T1",T1
    # s0.Get_Inv_10T_T01(T1)
    # print s0.GET_Inv_T(T1)
    ...
    while not rospy.is_shutdown():
        if len(struct_uv_sub.left_uv0_buf) != 0 and len(struct_uv_sub.left_uv1_buf) != 0 and len(
                struct_uv_sub.right_uv0_buf) != 0 and len(struct_uv_sub.right_uv1_buf) != 0:
            ...
            puv, theta_uv, puv_desire, theta_uv_desire, a_uv_desire, b_uv_desire = s0.Get_p_theta(n_uv, u1_desire,
                                                                                                  v1_desire, u2_desire,
                                                                                                  v2_desire, u1, v1, u2,
                                                                                                  v2)
            ...
            if len(q_now) != 0:
                ...
                print "q_now\n", q_now
                # 2,get ee_speed
                ee_speed = s0.Get_vxvyvzwxwywz(theta_uv, theta_uv_desire, puv, puv_desire, a_uv_desire, b_uv_desire,
                                               T_reflection_to_camera, T_camera_to_endeffector)
                # 3,get  joint speed
